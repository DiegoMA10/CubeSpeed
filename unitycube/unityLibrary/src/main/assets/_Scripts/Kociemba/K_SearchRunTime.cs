...
namespace Kociemba
{
    public class SearchRunTime
    {
        ...
        {
            // Check if tables are loaded, if not, warn the user
            if (!Tools.TablesLoaded)
            {
                info = "Error: Kociemba tables are not loaded. Please use KociembaTableLoader to load tables before solving.";
                return "Error: Tables not loaded";
            }

            info = "Using pre-loaded tables from StreamingAssets/Kociemba/Tables.";
            ...
            // +++++++++++++++++++++++ initialization +++++++++++++++++++++++++++++++++
            // Use CoordCube instead of CoordCubeBuildTables
            string dummyInfo;
            CoordCube c = new CoordCube(cc, DateTime.Now, "", out dummyInfo);
            //return "lol";
            ...
            do
            {
                ...
                mv = 3 * ax[n] + po[n] - 1;
                flip[n + 1] = CoordCubeTables.flip[flip[n],mv];
                twist[n + 1] = CoordCubeTables.twist[twist[n],mv];
                slice[n + 1] = CoordCubeTables.FRtoBR[slice[n] * 24,mv] / 24;
                minDistPhase1[n + 1] = Math.Max(CoordCube.getPruning(CoordCubeTables.Slice_Flip_Prun, CoordCube.N_SLICE1 * flip[n + 1] + slice[n + 1]), CoordCube.getPruning(CoordCubeTables.Slice_Twist_Prun, CoordCube.N_SLICE1 * twist[n + 1] + slice[n + 1]));
                // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        internal static int totalDepth(int depthPhase1, int maxDepth)
        {
            ...
            {
                mv = 3 * ax[i] + po[i] - 1;
                URFtoDLF[i + 1] = CoordCubeTables.URFtoDLF[URFtoDLF[i], mv];
                FRtoBR[i + 1] = CoordCubeTables.FRtoBR[FRtoBR[i], mv];
                parity[i + 1] = CoordCube.parityMove[parity[i]][mv];
            }

            if ((d1 = CoordCube.getPruning(CoordCubeTables.Slice_URFtoDLF_Parity_Prun, (CoordCube.N_SLICE2 * URFtoDLF[depthPhase1] + FRtoBR[depthPhase1]) * 2 + parity[depthPhase1])) > maxDepthPhase2)
            {
                return -1;
            }
            ...
            {
                mv = 3 * ax[i] + po[i] - 1;
                URtoUL[i + 1] = CoordCubeTables.URtoUL[URtoUL[i], mv];
                UBtoDF[i + 1] = CoordCubeTables.UBtoDF[UBtoDF[i], mv];
            }
            URtoDF[depthPhase1] = CoordCubeTables.MergeURtoULandUBtoDF[URtoUL[depthPhase1],UBtoDF[depthPhase1]];

            if ((d2 = CoordCube.getPruning(CoordCubeTables.Slice_URtoDF_Parity_Prun, (CoordCube.N_SLICE2 * URtoDF[depthPhase1] + FRtoBR[depthPhase1]) * 2 + parity[depthPhase1])) > maxDepthPhase2)
            {
                return -1;
            }
            ...
            do
            {
                ...
                URFtoDLF[n + 1] = CoordCubeTables.URFtoDLF[URFtoDLF[n], mv];
                FRtoBR[n + 1] = CoordCubeTables.FRtoBR[FRtoBR[n],mv];
                parity[n + 1] = CoordCube.parityMove[parity[n]][mv];
                URtoDF[n + 1] = CoordCubeTables.URtoDF[URtoDF[n], mv];

                minDistPhase2[n + 1] = Math.Max(CoordCube.getPruning(CoordCubeTables.Slice_URtoDF_Parity_Prun, (CoordCube.N_SLICE2 * URtoDF[n + 1] + FRtoBR[n + 1]) * 2 + parity[n + 1]), CoordCube.getPruning(CoordCubeTables.Slice_URFtoDLF_Parity_Prun, (CoordCube.N_SLICE2 * URFtoDLF[n + 1] + FRtoBR[n + 1]) * 2 + parity[n + 1]));
                // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
